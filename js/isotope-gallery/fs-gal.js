...
//the fs-gal plugin make the entire overlay carousel when the image is clicked.
...
$('document').ready(function() {
    ...
    function fsGal_DisplayImage(obj) {
        ...
        alt = obj.attr('alt');
        descr = obj.attr('descr');//added to display description in a p
        if (!title) { title = alt; }
        imgElem = $('.fs-gal-main');
        imgElem.attr('title', title);
        imgElem.attr('alt', alt);
        imgElem.attr('src', obj.attr('data-url'));
        $('.fs-gal-view > .text_overlay_title').text(title); //changed to display it in a div classed
        $('.fs-gal-view p').text(descr); //display in the p my description
        if (!title || title == '') { $('.fs-gal-view > .text_overlay_title').fadeOut(); }
        else { $('.fs-gal-view > .text_overlay_title').fadeIn(); }
...
//add dinamic title when display gallery
